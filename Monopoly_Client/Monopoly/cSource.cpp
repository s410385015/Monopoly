...
int main()
{
	...
	SockSet sock(ip);
	sock.Client();
	send(sock.GetSock(), p2n, MAXBYTE, NULL);
	nbyte = recv(sock.GetSock(), buffer, MAXBYTE, NULL);
	//buffer[nbyte] = '\0';
	system("Cls");
	game.Show_Theme();
	game.Show_Space();
	...
	//buffer[nbyte] = '\0';
	PlayerInfo pt;
	PlayerSet p1(buffer, 5, p1c);
	PlayerSet p2(p2n, 6, 5);
	...
	while (1)
	{
		...
		chosen[0] = '0' + t;
		con.ConDie(t);
		send(sock.GetSock(), chosen, MAXBYTE, NULL);
		game.Clear();
		...
		if (!data.IsHouse(p2.GetCurPos()))
		{
			...
			if (!temp.ifbuy){
				game.Show_Option(2);
				game.Show_HouseData(data.GetInfo(p2.GetCurPos()), 1);
				if (con.ChooseCon())
				{
					chosen[0] = '0' + 1;
					send(sock.GetSock(), chosen, MAXBYTE, NULL);
			...
			else
			{
				if (data.IsOwner(p2.GetCurPos(), pt.name)){
					if (temp.category != 4){
						game.Show_HouseData(data.GetInfo(p2.GetCurPos()), 3);
						game.Show_Option(3);
						if (con.ChooseCon())
						{
							chosen[0] = '0' + 1;
